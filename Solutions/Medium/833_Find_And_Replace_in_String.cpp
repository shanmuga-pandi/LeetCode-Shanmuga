...
public:
    ...
    string findReplaceString(string S, vector<int>& indexes, vector<string>& sources, vector<string>& targets) {
        unordered_map<int, pair<int, string>> replaced_list;
        int n = indexes.size();
        for(int i=0; i<n;i++) {
            string substr = S.substr(indexes[i], sources[i].size());
            if (substr == sources[i]) {
                replaced_list[indexes[i]] = {indexes[i]+substr.size(), targets[i]};
            }
        }
        ...
        string res;
        while(i < S.size()) {            
            if (replaced_list.count(i) > 0) {               
                res += replaced_list[i].second;
                i = replaced_list[i].first;   
            } else {                
                res += S[i];
                i++;
            }
